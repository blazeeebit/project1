...
export const onIntegrateDomain = async (domain: string, icon: string) => {
  ...
  try {
    ...
    // check for domain duplicates in the account
    ...
    //if domain does not exist create new domain under that account with a chatbot
    // create domain based on subscription plan
    if (!domainExists) {
      if (
...
export const onGetCurrentDomainInfo = async (domain: string) => {
  ...
  try {
    //get the domain info using the clerk id
